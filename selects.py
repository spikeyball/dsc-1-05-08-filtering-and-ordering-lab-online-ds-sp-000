...
def select_nameless_dog():
    return '''SELECT * FROM dogs WHERE name = "NULL";''' # change name back to * to test
...
def select_name_age_and_temperament_of_oldest_dog():
    return '''SELECT name, age, temperament FROM dogs ORDER BY age DESC LIMIT ONE;'''

def select_name_and_age_of_three_youngest_dogs():
    return '''SELECT name, age FROM dogs ORDER BY age LIMIT THREE;'''
...
def select_name_age_and_hungry_of_hungry_dogs_between_age_two_and_seven_in_alphabetical_order():
    return '''SELECT name, age, hungry FROM dogs WHERE hungry = 1 ABD age BETWEEN 2 AND 7 ORDER BY name;'''
